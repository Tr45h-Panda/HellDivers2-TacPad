...
uint8_t Touch_Read_Data(void) {
  ...
  for (i = 0; i < touch_cnt; i++) {
    ...
    uint16_t raw_x = (uint16_t)(((uint16_t)buf[(i * 5) + 2 + num] << 4) + ((buf[(i * 5) + 4 + num] & 0xF0) >> 4));               
    uint16_t raw_y = (uint16_t)(((uint16_t)buf[(i * 5) + 3 + num] << 4) + (buf[(i * 5) + 4 + num] & 0x0F));
    
    touch_data.coords[i].x = raw_y;           // X becomes Y
    touch_data.coords[i].y = 240 - raw_x;     // Y becomes (width - X)
